...
my_codegen_dir="${GEN_DIR}/generators/my-code-gen"
my_codegen="${my_codegen_dir}/target/my-codegen-openapi-generator-1.0.0.jar"
...
if [[ -f "${codegen}" && -f "${my_codegen}" && -n "$(java ${JAVA_OPTS} -cp ${my_codegen}:${codegen} org.openapitools.codegen.OpenAPIGenerator completion | grep "^$1\$" )" ]]; then
    command=$1
    shift
    exec java ${JAVA_OPTS} -cp ${my_codegen}:${codegen} org.openapitools.codegen.OpenAPIGenerator "${command}" "$@"
    ...
    if [[ ! -f "${my_codegen}" ]]; then
        (cd "${my_codegen_dir}" && exec mvn -Duser.home=$(dirname $MAVEN_CONFIG) package)
    fi
